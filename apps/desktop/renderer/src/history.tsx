...
/** The retained-history browser. A SETTINGS TAB since the header cleanup
 * (user 2026-09-10): the prominent header button gave low-priority history
 * a whole chrome seat, so the browser now renders inside the org Settings
 * modal's History tab — same data, same paging, no modal of its own. */
export function HistoryView({ slug }: { slug: string }) {
  ...
  // total against partial payloads: a stubbed or older backend must render
  // an empty browser, never crash the whole settings modal
  ...
  return <>
    ...
    {page && <>
      <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
        ...
        <span>{page.total ?? 0} records · page {cursors.length}</span>
        <button disabled={!page.next_cursor} onClick={() => setCursors(c => [...c, page.next_cursor!])}>Older</button>
      </div>
      {!page.items?.length && <p>No records in this collection.</p>}
