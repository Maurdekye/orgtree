...
function Fixture(){const [open,setOpen]=useState(true);Object.assign(window,{optionsProbe:{pin:()=>pinModal('docket',{x:30,y:30,w:500,h:650}),size:(w:number,h=650)=>commitModalRect('docket',{x:30,y:30,w,h})}});return <CurrentOrg.Provider value="fixture">{open?<DocketModal slug="fixture" tree={tree} toast={()=>{}} close={()=>setOpen(false)}/>:<div>Closed docket</div>}</CurrentOrg.Provider>}
