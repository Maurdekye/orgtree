// focusstick-probe.tsx — the page `focusstick_probe.py` drives in a REAL
...
// where there is no layout and no ResizeObserver: the canvas size is a number
// the test states and the "resize" is a fake observer callback it fires by
// hand. Two things therefore cannot be asked there, and they are the two the
// user actually described:
//   · does a REAL browser window resize reach the canvas at all — the real
//     ResizeObserver on the real element, at the real timings;
//   · is the focused desk still FITTED afterwards — its painted box inside
//     the free region — rather than merely aimed at by a camera whose numbers
//     nobody laid out.
//
// Nothing here asserts anything; the assertions live in the .py file. This
// file only has to be an honest host: the same component, the same
// stylesheet, no geometry of its own.
...
interface Probe {
  ...
  clearPins: () => void
  /** an org UPDATE that is not a resize: more agents, so the layout
   *  re-anchors and every card moves */
  setAgents: (ids: string[]) => void
  toasts: string[]
}
