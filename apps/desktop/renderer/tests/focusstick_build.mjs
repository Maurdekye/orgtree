// focusstick_build.mjs — bundles `focusstick-probe.tsx` (the REAL OrgCanvas
// from ../src, real styles.css) into a browser page for `focusstick_probe.py`.
// Step 1 of that two-step probe; kept out of the `*.test.tsx` glob because a
// bundle proves nothing on its own.
//
//   node tests/focusstick_build.mjs <outdir>
//   node tests/focusstick_build.mjs <outdir> --subst SUBST.json
//
...
const outdir = argv[0]
if (!outdir) {
  console.error('usage: node tests/focusstick_build.mjs <outdir> [--subst F]')
  process.exit(2)
}
...
await esbuild.build({
  entryPoints: [path.join(HERE, 'focusstick-probe.tsx')],
...
console.log(`focusstick_build: bundled${subst ? ' (subst=' + subst + ')' : ''} into ${outdir}`)
