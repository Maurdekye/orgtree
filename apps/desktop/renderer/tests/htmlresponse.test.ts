...
// AND against a "do nothing, DOMPurify already strips it" version.
//
// ⚠ SCOPE RULING: the transform is gated behind md()'s third argument,
// `agentHtmlResponse`, which the caller must explicitly pass `true` for —
// desk.tsx does that ONLY where the text is a genuine agent chat response
// (see the call-site comments there). Every test below that exercises the
// swap therefore calls `md(text, undefined, true)`, exactly mimicking that
// one call site — a test that omitted the flag would trivially pass once
// the flag defaults to off and would prove nothing. The final section
// tests the OFF path directly: the identical fenced text, without the
// flag, must stay an ordinary code block — that is the mail/user/tool/
...
test('⭐ the exact tag becomes a sandboxed iframe, not a code block', () => {
  const html = md(fence('orgtree-html-response', '<button onclick="go()">hi</button>'), undefined, AGENT).__html
...
test('⭐ the frame is sandboxed to allow-scripts ONLY — no allow-same-origin', () => {
  const html = md(fence('orgtree-html-response', '<p>hi</p>'), undefined, AGENT).__html
...
test('⭐ the raw markup never enters the OUTER document — only the frame\'s srcdoc', () => {
  ...
  const html = md(fence('orgtree-html-response', payload), undefined, AGENT).__html
...
test('an ordinary ```html sample is UNAFFECTED — explicit opt-in per BLOCK TAG, even with the agent grant on', () => {
  const html = md(fence('html', '<button onclick="go()">hi</button>'), undefined, AGENT).__html
...
test('plain prose and other fenced code are byte-for-byte unaffected, even with the agent grant on', () => {
  const prose = 'no code here, just **bold** text and a [link](https://example.com)'
  assert.ok(!md(prose, undefined, AGENT).__html.includes('<iframe'))
  const code = md(fence('python', 'print("hi")'), undefined, AGENT).__html
...
test('a message can mix ordinary prose with one html-response block', () => {
  ...
  const html = md(text, undefined, AGENT).__html
...
test('an oversized block falls back to an ordinary code block, not a frame', () => {
  ...
  const html = md(fence('orgtree-html-response', big), undefined, AGENT).__html
...
test('⭐ a literal, UNFENCED mention of the exact matching markup in prose does not become a real element', () => {
  ...
  // would fail if that upstream escaping ever regressed. Run WITH the
  // agent grant on — this must hold even at full permission, not because
  // the surface happens to be inert.
  const typed = 'just typing it out: <pre><code class="language-orgtree-html-response">x</code></pre>'
  const html = md(typed, undefined, AGENT).__html
  assert.ok(!html.includes('<iframe'), html)
...
test('a fence tag with trailing info-string text still matches — same rule as every other language', () => {
  ...
  const html = md(fence('orgtree-html-response ignored-note', '<b>hi</b>'), undefined, AGENT).__html
  assert.ok(html.includes('<iframe'), html)
})
...
test('a near-miss tag (prefix/suffix, not the exact language token) does NOT match', () => {
  for (const tag of ['orgtree-html-responsezzz', 'xorgtree-html-response', 'orgtree-html-respons']) {
    const html = md(fence(tag, '<b>hi</b>'), undefined, AGENT).__html
...
test('multiple html-response blocks in one message each get their own frame', () => {
  ...
  const tpl = document.createElement('template')
  tpl.innerHTML = md(text, undefined, AGENT).__html
...
// ------------------------------------------------------------- scope gate
// The identical fenced text, rendered WITHOUT the agent grant — this is
// exactly what mail.tsx, the user's own message, tool `cmd_out`, and the
// presented-document reader all do: call `md()` with no third argument.

test('⭐ WITHOUT the agent grant, the identical fence stays an ordinary code block — the mail/user/tool/document default', () => {
  const text = fence('orgtree-html-response', '<button onclick="go()">hi</button>')
  const html = md(text).__html
  assert.ok(!html.includes('<iframe'), `must not render live without the grant: ${html}`)
  assert.ok(html.includes('<pre'), html)
  assert.ok(html.includes('language-orgtree-html-response'), 'the tag is still visible as a labeled code sample, just inert')
})

test('⭐ the cache key separates the two modes — the SAME text does not leak its rendering across the grant', () => {
  const text = fence('orgtree-html-response', '<p>shared text</p>')
  const inert = md(text).__html
  const live = md(text, undefined, true).__html
  assert.notEqual(inert, live, 'agent-mode and inert renderings of identical text must never share a cache entry')
  assert.ok(!inert.includes('<iframe'), inert)
  assert.ok(live.includes('<iframe'), live)
  // and re-fetching each again (now both cached) still returns the right one
