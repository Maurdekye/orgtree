// mailsender_dump.mjs — bundles + runs `mailsender.dump.tsx`.
//
...
const out = path.join(HERE, '..', 'node_modules', '.orgtree-mailsender')
rmSync(out, { recursive: true, force: true })
mkdirSync(out, { recursive: true })

await esbuild.build({
  entryPoints: [path.join(HERE, 'mailsender.dump.tsx')],
  outdir: out,
  bundle: true,
  format: 'esm',
  platform: 'node',
...
if (!dest) {
  console.error('usage: node tests/mailsender_dump.mjs <out.html>')
  process.exit(2)
}
execFileSync(process.execPath, [path.join(out, 'mailsender.dump.mjs'), dest],
  { stdio: 'inherit' })
