...
import { PinFrame, forgetModalPins, pinModal, readModalPins } from '../src/canvas/modalpin'
...
test('org pin uses canvas coordinates, snaps to a desk peer, and joins its stacking band', async () => {
  ...
  pinModal('docket',{x:0,y:0,w:390,h:400})
  ...
  try {
    ...
    assert.equal(readModalPins().docket!.rect.x,10, 'right edge lands exactly on the peer left edge')
...
test('global dialogs have no pin or popout controls and overlap fading defaults off', async () => {
  localStorage.clear(); forgetModalPins()
  const v=await mountView(<CurrentOrg.Provider value="org"><ModalOverlapSettings/>{['app-settings','advanced-org','defaults','usage'].map(kind=><PinFrame key={kind} kind={kind} title={kind} panel="global-fixture" close={()=>{}}>content</PinFrame>)}</CurrentOrg.Provider>,el=>el)
  try {
    assert.equal(v.el.querySelectorAll('.global-fixture').length,4,'all global dialogs rendered')
