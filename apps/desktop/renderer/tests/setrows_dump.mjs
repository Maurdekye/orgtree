// setrows_dump.mjs — bundles + runs `setrows.dump.tsx` (D-222).
//
...
const out = path.join(HERE, '..', 'node_modules', '.orgtree-setrows')
rmSync(out, { recursive: true, force: true })
mkdirSync(out, { recursive: true })

await esbuild.build({
  entryPoints: [path.join(HERE, 'setrows.dump.tsx')],
  outdir: out,
  bundle: true,
  format: 'esm',
  platform: 'node',
...
if (!dest) {
  console.error('usage: node tests/setrows_dump.mjs <out.html> [--org]')
  process.exit(2)
}
execFileSync(process.execPath,
  [path.join(out, 'setrows.dump.mjs'), ...process.argv.slice(2)],
  { stdio: 'inherit' })
