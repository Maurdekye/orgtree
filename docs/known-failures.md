...
**During fence-off (coordinator decision 2026-09-26 13:58Z): the registry is declared failing again.** Fence-off
slices (S1-S11) land WITHOUT a registry commit. `test_state_operation_contracts`, `test_state_operation_inventory`
and the `test_state_*_boundary` suites are therefore expected to fail on span drift ("stale source span", "source
inventory binding is stale", a `KeyError` on a witness id, changed site counts) between batches; they stay declared
in `docs/test-baseline.json`. p01 re-anchors v3 once per batch of landings and once before the pre-release compare.
Any OTHER assertion failing in those modules is still a regression: read the message before calling it known.
