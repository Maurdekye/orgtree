...
branch, tested, independently reviewed and landed on main, then built together.
One of them is a behaviour change you will notice immediately — removing a
signed-in account no longer demands that you move its agents by hand first —
and the other four are things that were quietly wrong: your canvas position
when you switched organizations, the send-as-notice toggle leaking between chat
windows, the floating neighbor cards ignoring a right-click, and the window
controls drifting off the corner.

It arrives through one prerelease, 2.1.10-beta.0, which was built and run
locally before this release was cut. This stable build is byte-for-byte the
proven beta.0 contents; only the version and these notes differ.

## Upgrading

The tray's **Update now** will offer this build to an existing installation.
Builds on the 2.1.10 beta line update to it as well.
...
* The packaged `Orgtree.exe` reports a numeric Windows ProductVersion of
  `2.1.10.0`. That four-part field has no room for anything else; the
  installer, the executable's FileVersion, the application's own build
  information and the updater feed all read exactly `2.1.10`.
