...
Updated 2026-09-07 21:20 UTC. This is an independent evidence record, not a declaration of MVP completion. Binding scope is `v2-user-decisions.md` (newest decision wins), the retained requirements in `v2-original-design-brief.md`, and `v1-parity-inventory.md`. Technical wiring is described in `engine-contract.md`.
...
The runner creates a fresh `orgtree-v2-acceptance-*` directory under the OS temporary directory, outside the live v1 tree. It sets both data selectors and an isolated Electron profile **before any application import**. Both launches use that same fresh data/profile. It retains the data and structured `report.json`; it does not recursively delete anything. Raw application output is not printed. No acceptance case attaches to an existing engine or port 7360.
...
Engine owner subsequently reported local main `f17feeb` contains the launcher/backend and stable per-root port. Real application runs against that integrated build are pending. Shell owner accepted the draft-storage origin risk and coordinated stable origin retention with the engine owner. Until two actual launches pass, draft continuity remains unverified.

## Whole-app feature matrix

| Requirement | Acceptance boundary | Current evidence / remaining case |
|---|---|---|
| Fresh first launch, separate v2 root, copied Python source and real interpreter | Main spawns actual bundled interpreter; readiness root/PID/port must match; nonempty renderer must mount | Source inspected; real application pending |
| HTTP assets, REST, WebSocket and native bridge share the engine | Actual renderer loads; authenticated API succeeds and unauthenticated request fails | Runner prepared for renderer/REST/bridge; existing shell owner fixture covers WS/assets, full app WS pending |
| Organization create/select/settings/default project folder | Create isolated organization from visible UI, reload and verify persistence/default grant | Copied renderer source; interaction pending |
...
| Main close leaves engine and popouts; explicit Exit closes everything | Close real main, prove engine API still responds, reopen; repeat with actual desk/modal popout | Main case runnable; popout + Exit cleanup verification pending |
...
| Draft and event reply persistence | Draft text/attachment/reply storage survives two full engine launches; actual source identity/preview/send receipt survives reload/compaction | Storage transport case runnable with explicitly seeded draft values; real composer/source/send/compaction still pending |
...
| Real agent turn + MCP identity | ONE isolated Codex Luna/Astra arithmetic task and read-only chart, verifying MCP targets same test engine | Coordinator authorized exact bounded case 21:14 UTC; not yet run |
