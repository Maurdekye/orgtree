...
STATUS: HISTORICAL AND SUPERSEDED. This is the user's first design brief, discussion draft of 5 September 2026, as pasted by the user on 2026-09-07 at 20:13:47 UTC. It is preserved here word for word so the original intent is never lost. It is NOT the current specification: several of its decisions (multi-account management, the dedicated mobile app, selective parity, "no repository or implementation yet", open framework and engine choice) were changed by the user later on 7 September 2026. The current, binding record is `docs/v2-user-decisions.md`; where the two differ, that file wins. The desktop/engine transport is specified in `docs/engine-contract.md`.
