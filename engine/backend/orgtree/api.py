...
class Settings(Body):
    ...
    headless: bool | None = None            # §9.6: no user present (user
    ...
    if body.auto_cheap_compact is not None:
        ...
        if "enabled" in acc or "occ" in acc:
            ...
            org.d["auto_cheap_compact"] = {
                ...
                "occ": min(0.95, max(0.05, float(
                    acc.get("occ", old.get("occ", 0.5)))))}
    # ---- §9.6: headless (user redesign 2026-09-12: the V1 §9.5 per-org key
    # and its hard couplings are gone — inference credentials are
...
@app.get("/api/orgs/{slug}/documents/{did}/download")
def document_download(slug: str, did: str) -> Response:
    """Download the original presented source with a useful filename.

    Markdown is stored inline; HTML presentations are immutable outbox
    snapshots and are served as attachments, never executed by this route.
    """
    try:
        org = store.load_org(slug)
    except LedgerError as e:
...
def _agent_identity(body: AgentCall, request: Request, *, durable: bool = False) -> dict[str, Any]:
    ...
    with store.DOC_LOCK:
        ...
        # private parse. Authentication is the first of the three whole-document
        # loads every agent tool call used to pay for — 56-64 ms each on the
        # live org, measured 2026-09-18 — and it only ever reads. The snapshot
        # is guarded by `org_seq`, which every save bumps, so a seat archived or
        # regenerated by the previous call is seen by this one; and we are under
        # DOC_LOCK, so no write is half-applied underneath it.
        try:
            org = store.cached_org(body.org)
        ...
        if durable and not caller.get("seat_id"):
            ...
            #
            # ⚠ AND MINT ON A PRIVATE COPY. The snapshot above is shared with
            # every other reader in the process and must never be mutated or
            # saved — the same rule the incarnation minters follow. This branch
            # is the rare one (a legacy hire, once ever), so paying a real load
            # here costs nothing measurable and keeps the read path honest.
            org = store.load_org(body.org)
            caller = org.node(body.node)
