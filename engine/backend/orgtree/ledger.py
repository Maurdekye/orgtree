...
class Org:
    ...
    #: no progress argument preserves whatever the item already stored; when
    #: the item stored NOTHING, the summary would be empty and the update would
    #: be refused for saying nothing readable. This is what gets written
    #: instead — one line, generated by the backend rather than by the caller,
    #: naming the fact (it was staffed) and the next actor (who holds it now),
    #: which is the entire content of that update. It is an ordinary
    #: `working_on_next` entry afterwards: the owner's next update replaces it
    #: like any other, and nothing downstream treats it specially.
    STAFFING_BOUNDARY: Final = ('Staffed to "{node}" — it holds this item and '
                                'acts next.')
    ...
    def work_update(self, actor: str, wid: str, done_so_far: Any = None,
        ...
        ---- `staffed_to` IS NOT A CALLER ARGUMENT. It is set only by the
        composite staffing path (`api._staff_call`), never by `orgtree_work`,
        whose dispatch names every parameter it passes and does not name this
        one. It says "the substantive change in this update is that the item
        was STAFFED to that agent", and it buys exactly one thing: a staffing
        call that sends no progress argument at all preserves the stored
        summaries instead of being refused for not repeating them. Standalone
        updates are untouched — the rule that an update must say something the
        user can read is unchanged for every call that is not a staffing."""
        self._work_require_live_agent_or_user(actor)
        self._work_sweep()
        ...
        if no_progress and substantive:
            ...
            if staffed_to and not done and not nxt:
                ...
                # whom, which is the whole content of this update.
        ...
        return {"updated": wid, "rev": it["rev"], "status": it["status"],
                ...
                # the one line this call GENERATED rather than was given, when
                # a staffing found no stored summary to carry forward. Absent
                # on every other update, including a staffing that preserved
                # something — so its presence means exactly "the progress you
    ...
    def _work_evidence_row(self, actor: str, kind: str, ref: Any,
        ...
        if kind not in self.WORK_EVIDENCE_KINDS:
            msg = (f"evidence kind must be one of "
                   f"{'|'.join(self.WORK_EVIDENCE_KINDS)}")
            if errors is not None:
                errors.append(msg)
            else:
                raise LedgerError(f"{msg}{where}")
        r: str | None = None
        try:
            r = workfields.bounded("ref", ref)
            if not r:
                msg = "evidence needs a ref (path, url, sha, log name)"
                if errors is not None:
                    errors.append(msg)
                else:
                    raise LedgerError(f"{msg}{where}")
        ...
        try:
            meta = workevidence.validate_acceptance_evidence(
                ...
                composition=composition, errors=meta_errs)
        ...
        if meta_errs:
            if errors is not None:
                ...
                raise LedgerError(f"{'; '.join(meta_errs)}{where}")
        row.update(meta)
            ...
            if not isinstance(receipt, dict):
                ...
                if str(rc.get("schema") or "") != workevidence.SCHEMA:
                    msg = (
                        ...
                        f"only evidence while its shape is known")
                    if errors is not None:
                        errors.append(msg)
                    else:
                        raise LedgerError(f"{msg}{where}")
        ...
        if items is not None:
            ...
            for i, el in enumerate(batch):
                ...
                for err in el_errors:
                    all_errors.append(f"items[{i}]: {err}")
                rows.append(r)
            if all_errors:
                raise LedgerError("; ".join(all_errors))
        else:
            single_errors: list[str] = []
            row = self._work_evidence_row(
                actor, kind, ref, note,
            ...
            if single_errors:
                raise LedgerError("; ".join(single_errors))
            rows.append(row)
    ...
    def _work_batch_arg(raw: Any, name: str) -> list[dict[str, Any]]:
        ...
        for i, el in enumerate(out):
            if not isinstance(el, dict):
                errs.append(f"`{name}`[{i}] must be an object, not "
                            f"{type(el).__name__}")
        if errs:
    ...
    def _work_check_one(self, actor: str, acc: list[Any], index: Any,
        ...
        try:
            i = int(index)
            if not 0 <= i < len(acc):
                msg = f"acceptance index {i} out of range (0..{len(acc) - 1})"
                if errors is not None:
                    errors.append(msg)
                else:
                    raise LedgerError(f"{msg}{where}")
            ...
            msg = "acceptance `index` must be an integer (0-based)"
            if errors is not None:
                errors.append(msg)
            else:
                raise LedgerError(f"{msg}{where}") from None
        r: str | None = None
        try:
            r = workfields.bounded("evidence_ref", evidence_ref)
            if not r:
                msg = "checking a condition needs an evidence_ref"
                if errors is not None:
                    errors.append(msg)
                else:
                    raise LedgerError(f"{msg}{where}")
        ...
        if checks is not None:
            ...
            for n, el in enumerate(batch):
                ...
                if i is not None:
                    if i in seen:
                        el_errors.append(
                            f"marks acceptance index {i} again — one batch may not "
                            f"check the same condition twice, since only one of the "
                            f"two evidence refs could survive")
                    else:
                        seen.add(i)
                for err in el_errors:
                    all_errors.append(f"checks[{n}]: {err}")
                if i is not None and not el_errors:
                    done.append((i, rec))
            if all_errors:
                raise LedgerError("; ".join(all_errors))
        else:
