...
TOOLS: list[dict[str, Any]] = [
        ...
        "inputSchema": {
            ...
            "properties": {
                ...
                "classification": {"type": "string", "enum": ["met", "not_exercised", "environment_limited", "known_negative"],
                                    "description": "check/evidence: explicit acceptance evidence class. met and a successful known_negative are completed conditions; not_exercised and environment_limited remain qualified evidence."},
                "artifact": {"type": "string", "description": "check/evidence: the exact artifact name or path measured." + _NOCAP},
                "runner": {"type": "string", "description": "check/evidence: runner or interpreter identity that produced or reported the result." + _NOCAP},
                "gate": {"type": "string", "description": "check: exact blocked-request gate exercised by a known-negative control." + _NOCAP},
                "blocked_count": {"type": "integer", "minimum": 0, "description": "check: exact number of requests blocked by the named gate."},
                ...
                "checks": {"type": "array", "items": {"type": "object"},
                           "description": "check: a BATCH — [{index, evidence_ref, classification, artifact, runner, execution, result, gate, blocked_count, composition, note}] — instead of one index. ATOMIC: every element is validated before any is written, so one bad index changes nothing, and the whole batch writes ONE history row so it reads as a single completion event rather than four. Cannot be combined with single-check fields, and the same index twice in one batch is refused"},
    ...
    },
    {
        ...
        "description": (
            ...
            "changes. Authority is re-derived from the new ancestry, so your "
