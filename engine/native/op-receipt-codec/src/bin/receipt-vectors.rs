...
fn main() {
    ...
    let (source, text) = match args.as_slice() {
        ...
        _ => {
            eprintln!("usage: receipt-vectors [PATH]");
            std::process::exit(2);
        }
    };
    ...
    println!(
        r#"{{"source":{},"rows":{},"sections":{{{}}},"failures":[{}],"unresolved":[{}]}}"#,
        json_str(&source),
        report.total(),
        sections.join(","),
        failures.join(","),
