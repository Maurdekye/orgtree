...
MUTANTS = [
    ...
    ("migrate.cr_kept", "src/migrate.rs",
     ...
     "    text.to_string()",
     ["--test", "migrate"], False),
    ("acl.unprotected_dacl_accepted", "../prototype-guard/src/acl.rs",
     "    r.owner == operator_sid && r.protected && !r.any_inherited",
     "    r.owner == operator_sid && !r.any_inherited",
     ["--test", "acl"], False),
    ("acl.inherited_aces_accepted", "../prototype-guard/src/acl.rs",
     "    r.owner == operator_sid && r.protected && !r.any_inherited",
     "    r.owner == operator_sid && r.protected",
     ["--test", "acl"], False),
    ("acl.extra_trustee_accepted", "../prototype-guard/src/acl.rs",
    ...
    ("pidfile.status_line_ignored", "src/cluster.rs",
     ...
     "    let _ = text;\n    None",
     ["--test", "pidfile"], False),
    ("start.any_lock_is_stale", "src/cluster.rs",
     "        (Some(created), Some(started)) => created > started + 1,\n        _ => false,",
     "        (Some(created), Some(started)) => created > started + 1,\n        _ => true,",
     ["--test", "pidfile"], False),
    ("start.reused_pid_not_stale", "src/cluster.rs",
     "        (Some(created), Some(started)) => created > started + 1,",
