//! Owner-only files and folders (shared by the WS1 custodian and the WS2 store
//! service, so both apply ONE rule to secrets and descriptors), on the
...
mod imp {
    ...
    fn last(code: &'static str, what: &str) -> GuardError {
        GuardError::new(code, format!("{what}: {}", std::io::Error::last_os_error()))
    }
    ...
    pub fn read_acl(path: &Path) -> Result<AclReport> {
        ...
        if rc != 0 {
            return Err(GuardError::new("acl.read", format!("{}: GetNamedSecurityInfoW error {rc}", path.display())));
        }
        let mut s: *mut u16 = std::ptr::null_mut();
...
mod imp {
    use super::*;
    fn no<T>() -> Result<T> {
        Err(GuardError::new("acl.unsupported", "owner-only ACLs are Windows-only in P03"))
    }
    pub fn current_user_sid() -> Result<String> {
        no()
    }
...
pub fn require_owner_only(path: &Path) -> Result<AclReport> {
    ...
    if !is_owner_only(&r, &sid) {
        return Err(GuardError::new("acl.not_owner_only", format!("{} has DACL {}", path.display(), r.sddl)));
    }
    Ok(r)
}
...
pub fn replace_owner_only(target: &Path, bytes: &[u8]) -> Result<()> {
    let tmp = target.with_extension(format!("tmp-{}-{}", std::process::id(), std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).map(|d| d.as_nanos()).unwrap_or(0)));
    write_owner_only_file(&tmp, bytes)?;
    require_owner_only(&tmp)?;
    std::fs::rename(&tmp, target).map_err(|e| GuardError::new("acl.rename", format!("{}: {e}", target.display())))?;
