...
| `agent_config`, `configuration_intents`, charter heads/versions | `charter_heads`, `charter_versions` (keyed by `charter_kind` role/team: freeze amendment 1) | charters met: one head per principal and kind. `agent_config` / `configuration_intents` are **absent**: queued config is excluded from P03 (coordinator ruling Q7). |
...
| `runtime_state`, `runtime_claims`, `input_batches`, `input_evidence` | `runtime_state` (busy, bg_open; NO status: WS4 0200 owns the status row, freeze amendment 1), `runtime_claims` (one active per seat), `runtime_inflight` (one row per call), `service_incarnations` | minimal runtime met. `input_batches`/`input_evidence` are **absent**; WS5 adds `mail_input_batches` (0300, relayed from WS5). |
