#![allow(dead_code)]
//! WS3b copy of WS5 common_pg (only OWN_CLUSTER and the org slug differ;
...
// (WS5 stand-ins not used: WS3b tests race the real writers)

const OWN_CLUSTER: &str = "\\artifacts\\p03-db\\p03-ws3b-topology\\";
...
pub async fn reset() {
    ...
    let o = org();
    let mut sql = format!(
        "INSERT INTO organizations (org_id, slug, created_at) VALUES ('{o}', 'p03-ws3b', now());
