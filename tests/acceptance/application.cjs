...
app.on('browser-window-created', (_event, main) => {
  main.webContents.once('did-finish-load', async () => {
    ...
    await check('global-settings-and-org-modal-popout-redock-main-close', async () => {
      ...
      assert.equal(await evaluate(`Boolean([...document.querySelectorAll('.acct-panel button')].find(b=>b.textContent==='close'))`), true, 'App settings close button exists')
      await evaluate(`[...document.querySelectorAll('.acct-panel button')].find(b=>b.textContent==='close').click(); true`)
      assert.equal(await waitFor(`!document.querySelector('.acct-panel')`), true)
      assert.equal(await evaluate(`Boolean(document.querySelector('header.orgbar button[title*="usage"]'))`), true, 'Header Usage action exists')
      await evaluate(`document.querySelector('header.orgbar button[title*="usage"]').click(); true`)
      assert.equal(await waitFor(`document.querySelector('.settings.usage-modal button[aria-label="Open in new window"]')`), true, 'Org panel retains popout action')
      const before = new Set(BrowserWindow.getAllWindows().map(w => w.id))
      const mainStyle = await evaluate(`(()=>{const s=getComputedStyle(document.querySelector('.settings.usage-modal'));return {background:s.backgroundColor,color:s.color,font:s.fontFamily}})()`)
      ...
      await check('native-popout-retains-stylesheets', async () => {
        ...
        assert.deepEqual(styleState.panel, mainStyle, 'Adopted Usage must retain its rendered styles')
      })
      await check('capture-native-org-popout', () => capture(child, 'usage-popout'))
      main.close()
      ...
      assert.equal(await waitFor(`!document.querySelector('.settings.usage-modal')`), true)
    })
