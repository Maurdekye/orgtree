...
app.on('browser-window-created', (_event, main) => {
  main.webContents.once('did-finish-load', async () => {
    ...
    await check('global-settings-and-org-modal-popout-redock-main-close', async () => {
      ...
      assert.equal(await waitFor(`document.querySelector('.settings.wide button[aria-label="Open in new window"]')`), true, 'Org panel retains popout action')
      const before = new Set(BrowserWindow.getAllWindows().map(w => w.id))
      const mainStyle = await evaluate(`(()=>{const s=getComputedStyle(document.querySelector('.settings.wide'));return {background:s.backgroundColor,color:s.color,font:s.fontFamily}})()`)
      ...
      await evaluate(`[...document.querySelectorAll('.settings.wide button')].find(b=>b.textContent==='close').click(); true`)
    })
