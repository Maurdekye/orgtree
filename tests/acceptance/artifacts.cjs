...
function finish(error){if(finished)return;finished=true;clearTimeout(deadline);if(error)checks.push({name:'artifact-flow',status:'FAIL',reason:error.message});fs.writeFileSync(path.join(root,'artifacts.json'),JSON.stringify({status:error?'FAIL':'PASS',checks,screenshots,childPids:child?[child.pid]:[],limits:['Synthetic files/scoped actor; no provider turn','Actual snapshot API, native downloads and restricted artifact window']},null,2));app.quit()}
...
app.on('browser-window-created',(_e,main)=>{if(started)return;started=true;main.webContents.once('did-finish-load',async()=>{
  ...
  try{
    ...
    await capture(artifact,'restricted-html-viewer');checks.push({name:'real-artifact-viewer-no-bridge-or-api-authority',status:'PASS',apiAttempt:blocked})
    artifact.close();finish()
