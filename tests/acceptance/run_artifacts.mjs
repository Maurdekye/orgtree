...
const env=acceptanceEnvironment(root,{env:{ORGTREE_ACCEPTANCE_APP:resources,ORGTREE_ACCEPTANCE_VISUAL_FIXTURE:'1',ORGTREE_V2_PYTHON:python,ORGTREE_V2_PORT:'0',...(packaged?{ORGTREE_ACCEPTANCE_PACKAGE:packaged}:{})}})
assertIsolatedEnvironment(env,root)
const result=spawnSync(electron,acceptanceLaunchArgs(path.join(here,'artifacts.cjs')),{cwd:target,env,encoding:'utf8',windowsHide:true,timeout:100000,maxBuffer:1024*1024})
