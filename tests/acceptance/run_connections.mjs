...
const exchange=fs.mkdtempSync(path.join(os.tmpdir(),'v2-local-connections-')),manifest=runtimeManifest(target)
async function run(role){
  const root=isolatedRoot(),home=path.join(root,'home');fs.mkdirSync(home)
  const env={...process.env,HOME:home,USERPROFILE:home,ORGTREE_ACCEPTANCE_ROOT:root,ORGTREE_ACCEPTANCE_APP:target,ORGTREE_ACCEPTANCE_VISUAL_FIXTURE:'1',ORGTREE_ACCEPTANCE_HUB_ROLE:role,ORGTREE_ACCEPTANCE_EXCHANGE:exchange,ORGTREE_DATA:path.join(root,'inherited-v1'),ORGTREE_V2_DATA:path.join(root,'data'),ORGTREE_V2_PROFILE:path.join(root,'profile'),ORGTREE_V2_PYTHON:python,ORGTREE_V2_PORT:'0'}
  ...
  const child=spawn(electron,[path.join(here,'connections.cjs')],{cwd:target,env,windowsHide:true,stdio:['ignore','ignore','ignore']})
