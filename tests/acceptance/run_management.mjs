...
const manifest=runtimeManifest(target),env={...process.env,HOME:process.env.USERPROFILE,USERPROFILE:process.env.USERPROFILE,ORGTREE_ACCEPTANCE_ROOT:root,ORGTREE_ACCEPTANCE_APP:target,ORGTREE_ACCEPTANCE_VISUAL_FIXTURE:'1',ORGTREE_DATA:path.join(root,'inherited-v1'),ORGTREE_V2_DATA:path.join(root,'data'),ORGTREE_V2_PROFILE:path.join(root,'profile'),ORGTREE_V2_PYTHON:python,ORGTREE_V2_PORT:'0'}
...
const result=spawnSync(electron,[path.join(here,'management.cjs')],{cwd:target,env,encoding:'utf8',windowsHide:true,timeout:100000,maxBuffer:1024*1024})
const output=path.join(root,'management.json'),report=fs.existsSync(output)?JSON.parse(fs.readFileSync(output,'utf8')):{status:'FAIL',reason:'No application report'}
