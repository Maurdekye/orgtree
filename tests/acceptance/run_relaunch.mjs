...
const root=isolatedRoot(),manifest=runtimeManifest(target),env={...process.env,ORGTREE_ACCEPTANCE_ROOT:root,ORGTREE_ACCEPTANCE_APP:target,ORGTREE_ACCEPTANCE_VISUAL_FIXTURE:'1',ORGTREE_DATA:path.join(root,'inherited-v1'),ORGTREE_V2_DATA:path.join(root,'data'),ORGTREE_V2_PROFILE:path.join(root,'profile'),ORGTREE_V2_PYTHON:python,ORGTREE_V2_PORT:'0'}
for(const name of ['ELECTRON_RUN_AS_NODE','ORGTREE_PORT','ORGTREE_V1_ROOT','ORGTREE_V2_TOKEN','ORGTREE_ACCEPTANCE_IMPORT_FIXTURE'])delete env[name]
fs.writeFileSync(path.join(root,'runtime-manifest.json'),JSON.stringify(manifest,null,2))
const started=Date.now(),result=spawnSync(electron,[path.join(here,'relaunch.cjs')],{cwd:target,env,windowsHide:true,encoding:'utf8',timeout:60000,maxBuffer:1024*1024})
