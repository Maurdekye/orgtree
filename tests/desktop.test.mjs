...
test('preferences default close-to-tray/login and retain explicit off across reload', () => {
  ...
  assert.deepEqual(prefs.get(), { visualTheme: 'orgtree', contrastTheme: 'charcoal', visualThemeExplicit: false, exitOnClose: false, startAtLogin: true, automaticUpdates: true, routineNotifications: false, onboarded: false })
  prefs.set({ exitOnClose: true, startAtLogin: false })
  assert.deepEqual(new Preferences(file).get(), { visualTheme: 'orgtree', contrastTheme: 'charcoal', visualThemeExplicit: false, exitOnClose: true, startAtLogin: false, automaticUpdates: true, routineNotifications: false, onboarded: false })
...
test('all themes persist; invalid themes reject atomically and old preferences migrate', () => {
  ...
  for (const visualTheme of ['orgtree','claude','codex','antigravity','openrouter']) {
    ...
    assert.deepEqual(new Preferences(file).get(), {visualTheme,contrastTheme:'charcoal',visualThemeExplicit:true,startAtLogin:false,exitOnClose:true,automaticUpdates:true,routineNotifications:false,onboarded:false})
  }
