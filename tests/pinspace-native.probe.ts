...
app.whenReady().then(async()=>{
  ...
  const bad=await w.webContents.executeJavaScript(`(()=>{const p=document.querySelector('.fixture-gallery').parentElement;const z=p.style.zIndex;p.style.zIndex='99';const r=document.getElementById('hud-button').getBoundingClientRect();const hit=document.elementFromPoint(r.x+r.width/2,r.y+r.height/2)?.id;p.style.zIndex=z;return hit})()`)
