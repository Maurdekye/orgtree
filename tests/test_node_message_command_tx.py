...
audience). What must hold:
  ...
  4. CHANGED FAILURE BEHAVIOUR, pinned: if that send raises, the notices
     stay committed, the failure is reported as a warning, and the send is
     not retried.
...
class CommandTx(unittest.TestCase):
    ...
    def test_a_failed_send_keeps_the_notices_and_says_so(self):
        """The CHANGED failure behaviour, pinned: the deep reach is durable,
        the failure is a warning, and nothing is retried."""
        self.halt()
        ...
        self.assertIn('the command was not delivered: boom', r['warnings'][0])
        self.assertEqual(self.reached(), (1, True))
