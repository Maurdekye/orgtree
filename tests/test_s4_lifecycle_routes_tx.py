...
SQLite root:
  * each route commits through exactly one org_tx, in exactly one attempt
    when nothing contends, and broadcasts as many times as the legacy route
    did (the commit's save hooks plus the route's own call);
...
class RoutesCommitOnce(unittest.TestCase):
    ...
    def test_reorder(self):
        ...
        self.assertEqual((c.attempts, c.commits, c.hub), (1, 1, 2))
