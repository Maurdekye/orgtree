...
class SteerIdleFastPathTests(unittest.TestCase):
    ...
    def test_idle_fetch_opens_no_transaction(self):
        with patch.object(orgtx, "org_tx", wraps=orgtx.org_tx) as tx, \
                patch.object(orgtx, "org_read", wraps=orgtx.org_read) as read, \
            ...
            self.assertGreater(tx.call_count + read.call_count + load.call_count, 0)
            tx.reset_mock()
            read.reset_mock()
            load.reset_mock()
            for i in range(20):
            ...
            self.assertEqual(read.call_count, 0, "an idle fetch read the org")
    ...
    def test_attempt_written_during_a_scan_is_not_skipped(self):
        ...
        def read_then_race(*a, **kw):
            snap = real(*a, **kw)               # this read sees nothing open
            if not injected:
                injected.append(("pending", []))
                injected[0] = sup.claim_steer(self.slug, self.nid, "tool-r", self.tp)
            return snap

        with patch.object(orgtx, "org_read", side_effect=read_then_race):
            sup.scan_steer_records(self.slug, self.nid)
        did = injected[0][0]
