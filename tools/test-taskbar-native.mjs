...
import {configureTaskbar} from './apps/desktop/main/taskbar'
import {spawn} from 'node:child_process'
...
app.whenReady().then(async()=>{
 ...
 configureTaskbar(w,process.execPath,${JSON.stringify(icon)})
 const after=await read();assert.equal(after['5'],'com.maurdekye.orgtree');assert.equal(after['2'],'"'+process.execPath+'"');assert.equal(after['4'],'Orgtree');assert.ok(after['3'].startsWith(${JSON.stringify(icon)}))
