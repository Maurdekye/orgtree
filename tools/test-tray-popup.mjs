...
const root = fs.mkdtempSync(path.join(os.tmpdir(), 'orgtree-tray-popup-'))
await build({ entryPoints: ['tests/tray-popup-native.probe.ts'], outfile: path.join(root, 'probe.cjs'), bundle: true, format: 'cjs', platform: 'node', external: ['electron'] })
