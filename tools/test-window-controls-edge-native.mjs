...
const root = fs.mkdtempSync(path.join(os.tmpdir(), 'orgtree-edge-native-'))

// Fixture mirrors App.tsx's native header structure inside <main class="solo">
await build({
  stdin: {
    contents: `
...
const chips = Array.from({ length: 3 }, (_, i) =>
  e('span', { className: 'chip', key: i }, 'status ' + i))
createRoot(document.getElementById('root')).render(
  e('main', { className: 'solo' },
    e('header', { id: 'native', className: 'orgbar native-header' },
      e('div', { className: 'native-header-main' },
        ...
        e('button', { className: 'iconbtn' }, 'Settings')),
      e('div', { className: 'update-notice' }, 'Update ready'),
      e(WindowControls, null)),
    e('div', { className: 'canvas-stage' },
      e('div', { className: 'window-drag-margin' }),
      e('div', { className: 'viewport' }, 'Canvas Area'),
      e('div', { className: 'window-drag-margin' }))))
`,
    loader: 'tsx',
    resolveDir: process.cwd(),
  },
  outfile: path.join(root, 'fixture.js'),
  bundle: true,
  platform: 'browser',
  format: 'iife',
  jsx: 'automatic',
...
await build({
  entryPoints: ['tests/window-controls-edge-native.probe.ts'],
  outfile: path.join(root, 'probe.cjs'),
  bundle: true,
  platform: 'node',
  format: 'cjs',
  external: ['electron'],
